...
//		login.getLogin(var_adminUserName, var_adminPass);
		...
		ExecutionLog.log(ExecutionLog.color("red", "BUG ID: https://jira.tools.thcoe.ca/browse/APTWAM-396"));
		//verifyPresenceOfElement("//td[contains(text(),'" + var_paymentTypeSearch + "')] | //span[contains(text(),'" + var_paymentTypeSearch +"')]", var_paymentTypeSearch);
		verifyPresenceOfElement("//td[contains(text(),'" + status +"')] | //span[contains(text(),'"+status +"')]", status);
